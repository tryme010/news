...
def _assign_rss_topic(item: Dict, topics: List[Dict]) -> Dict | None:
    ...
    """
    # Prefer an explicit feed-provided topic when it matches a configured
    # topic. This is more reliable than keyword matching for broad world-news
    # headlines that may not contain the topic name itself.
    feed_topic_hint = str(item.get("feed_topic_hint", "")).strip()
    if feed_topic_hint:
        for topic in topics:
            if str(topic.get("id", "")).strip() == feed_topic_hint:
                return topic
